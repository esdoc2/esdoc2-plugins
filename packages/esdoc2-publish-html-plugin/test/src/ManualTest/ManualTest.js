...
describe('test manual', ()=>{
  ...
  describe('test each manual', ()=>{
    it('has overview', ()=>{
      ...
      assert.includes(doc, '.github-markdown[data-ice="content"]', 'esdoc2 is a documentation generator for JavaScript(ES6).');
    });

    it('has installation', ()=>{
      const doc = readDoc('manual/installation.html');
      assert.includes(doc, '.github-markdown h1', 'Installation');
      assert.includes(doc, '.github-markdown[data-ice="content"]', 'npm install -g esdoc2');
    });

    it('has usage', ()=>{
      const doc = readDoc('manual/usage1.html');
      assert.includes(doc, '.github-markdown h1:nth-of-type(1)', 'Usage');
      assert.includes(doc, '.github-markdown[data-ice="content"]', 'esdoc2 -c esdoc.json');
    });
    ...
    it('has faq', ()=>{
      ...
      assert.includes(doc, '.github-markdown[data-ice="content"]', 'esdoc2 has two goals.');
    });
